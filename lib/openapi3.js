    ...
    if (op.requestBody) {
        for (var rb in op.requestBody.content) {
            ...
            if (op.requestBody["x-widdershins-oldRef"]) {
                bp.refName = op.requestBody["x-widdershins-oldRef"].replace('#/components/requestBodies/','');
            }

            bp.schema = op.requestBody.content[rb].schema;

            if (op.requestBody.content[rb].examples) {
                let key = Object.keys(op.requestBody.content[rb].examples)[0];
                bp.exampleValues.object = op.requestBody.content[rb].examples[key].value;
                bp.exampleValues.description = op.requestBody.content[rb].examples[key].description;
            } else {
                bp.exampleValues.object = common.getSample(op.requestBody.content[rb].schema,data.options,{skipReadOnly:true},data.api);
            }

            if (typeof bp.exampleValues.object === 'object') {
                bp.exampleValues.json = safejson(bp.exampleValues.object,null,2);
            } else {
                bp.exampleValues.json = bp.exampleValues.object;
            }

            data.bodyParameters.push(bp);
        }
    }
}
    ...
    if (effSecurity && effSecurity.length && data.api.components && data.api.components.securitySchemes) {
        for (let ess of effSecurity) {
            if (data.api.components.securitySchemes[ess]) {
                ...
                if (!existingAuth && ((secScheme.type === 'oauth2') || (secScheme.type === 'openIdConnect') ||
                ...
                else if (secScheme.type === 'http' && secScheme.in === 'header') {
                    let authHeader = {};
                    authHeader.name = secScheme.name;
                    authHeader.type = 'string';
                    authHeader.in = 'header';
                    authHeader.isAuth = true;
                    authHeader.exampleValues = {};
                    authHeader.exampleValues.object = secScheme.scheme + ' {token}';
                    authHeader.exampleValues.json = "'" + authHeader.exampleValues.object + "'";
                    data.allHeaders.push(authHeader);
                }
            }
...
function getBodyParameterExamples(data) {
    ...
    data.bodyParameters.forEach(bodyParameter => {
        let obj = bodyParameter.exampleValues.object;
        let xmlWrap = false;
        if (bodyParameter.schema && bodyParameter.schema.xml) {
            xmlWrap = bodyParameter.schema.xml.name;
        } else if (bodyParameter.schema && bodyParameter.schema["x-widdershins-oldRef"]) {
            xmlWrap = bodyParameter.schema["x-widdershins-oldRef"].split('/').pop();
        }
...
function fakeBodyParameter(data) {
    ...
    data.bodyParameters.forEach(bodyParameter => {
        ...
        if ((param.schema.type === 'object') && (data.options.expandBody || (!param.schema["x-widdershins-oldRef"]))) {
            ...
            let props = common.schemaToArray(bodyParameter.schema,offset,{trim:true},data);
